(function registerDashboardWidgets() {
  ...
  function normalize(value) { const input = Array.isArray(value) ? value.filter((item) => item && typeof item.id === "string") : []; const byId = new Map(input.map((item) => [item.id, item])); const savedIds = [...new Set(input.map((item) => item.id).filter((id) => definitions.some((definition) => definition.id === id)))]; const orderedIds = [...savedIds, ...definitions.map((item) => item.id).filter((id) => !byId.has(id))]; return orderedIds.map((id) => { const definition = definitions.find((item) => item.id === id); const saved = byId.get(id); return { id, visible: saved?.visible !== false, size: ["small", "medium", "large"].includes(saved?.size) ? saved.size : definition.size }; }); }
  function init() {
  ...
  function bind() {
    ...
    grid.addEventListener("dragstart", (event) => { if (!editing || !event.target.matches(".widget-drag-handle")) return event.preventDefault(); clearPointerDrag(); clearDragFeedback(); event.target.closest("[data-widget-id]").classList.add("is-dragging"); event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", event.target.closest("[data-widget-id]").dataset.widgetId); });
    grid.addEventListener("dragover", (event) => { if (!editing) return; event.preventDefault(); markDropTarget(event.target.closest("[data-widget-id]")); });
    grid.addEventListener("drop", (event) => { event.preventDefault(); reorder(event.dataTransfer.getData("text/plain"), event.target.closest("[data-widget-id]")?.dataset.widgetId); clearDragFeedback(); });
    grid.addEventListener("dragend", clearDragFeedback);
    grid.addEventListener("pointerdown", (event) => { if (editing && event.target.matches(".widget-drag-handle")) { pointerDragId = event.target.closest("[data-widget-id]").dataset.widgetId; pointerDragTargetId = pointerDragId; clearDragFeedback(); event.target.closest("[data-widget-id]").classList.add("is-dragging"); } });
    grid.addEventListener("pointermove", (event) => { if (!pointerDragId) return; const target = document.elementFromPoint(event.clientX, event.clientY)?.closest("[data-widget-id]"); if (target) { pointerDragTargetId = target.dataset.widgetId; markDropTarget(target); } });
    window.addEventListener("pointerup", finishPointerDrag);
    window.addEventListener("pointercancel", clearPointerDrag);
  }
  function ensureQuickActions(home) { const quickGrid = home.querySelector(".dashboard-widget-grid"); if (!quickGrid || quickGrid.querySelector('[data-quick-action="layouts"]')) return; const button = document.createElement("button"); button.className = "dashboard-widget"; button.type = "button"; button.dataset.quickAction = "layouts"; button.innerHTML = '<span class="widget-icon" aria-hidden="true"><svg class="lucide-icon" viewBox="0 0 24 24"><rect width="7" height="7" x="3" y="3" rx="1"/><rect width="7" height="7" x="14" y="3" rx="1"/><rect width="7" height="7" x="3" y="14" rx="1"/><rect width="7" height="7" x="14" y="14" rx="1"/></svg><span class="widget-code">LAY</span></span><strong>공간·레이아웃</strong><small>연회장 공간과 도면을 관리하세요.</small>'; button.onclick = () => document.querySelector('.sidebar-nav-item[data-dashboard-target="layouts"]')?.click(); quickGrid.append(button); }
  function clearDragFeedback() { grid?.querySelectorAll(".is-dragging,.is-drop-target").forEach((shell) => shell.classList.remove("is-dragging", "is-drop-target")); }
  function markDropTarget(shell) { grid?.querySelectorAll(".is-drop-target").forEach((item) => item.classList.remove("is-drop-target")); if (shell && !shell.classList.contains("is-dragging")) shell.classList.add("is-drop-target"); }
  function clearPointerDrag() { pointerDragId = ""; pointerDragTargetId = ""; }
  function finishPointerDrag() { if (pointerDragId) reorder(pointerDragId, pointerDragTargetId); clearPointerDrag(); clearDragFeedback(); }
