(function registerDashboardWidgets() {
  ...
  function bind() {
    ...
    grid.addEventListener("dragstart", (event) => { if (!editing || !event.target.matches(".widget-drag-handle")) return event.preventDefault(); pointerDragId = ""; pointerDragTargetId = ""; event.dataTransfer.setData("text/plain", event.target.closest("[data-widget-id]").dataset.widgetId); });
  ...
  function finishPointerDrag() { if (pointerDragId) reorder(pointerDragId, pointerDragTargetId); clearPointerDrag(); }
